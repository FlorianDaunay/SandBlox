...
function createVoxelTexture(baseColorHex, forceNoBorder = false) {
            ...
            for (let z = 0; z < MAP_SIZE; z++) {
                ...
                if (blockId !== AIR) {
                    ...
                    if (blockId === COBBLESTONE)
                        instancedCobblestoneMesh.setMatrixAt(cIdx++, dummy.matrix);
                    else if (blockId === GRASS)
                        instancedGrassMesh.setMatrixAt(gIdx++, dummy.matrix);
                    else if (blockId === DIRT)
                        instancedDirtMesh.setMatrixAt(dIdx++, dummy.matrix);
                    else if (blockId === WATER)
                        instancedWaterMesh.setMatrixAt(wIdx++, dummy.matrix);
                    else if (blockId === BOOST)
                        instancedBoostMesh.setMatrixAt(bIdx++, dummy.matrix);
                }
            }
        }
    }
    ...
    for (const id in serverPlayers) {
        if (id !== socket.id && remotePlayers[id]) {
            ...
            if (mapData && rGridX >= 0 && rGridX < MAP_SIZE && rGridY >= 0 && rGridY < MAX_HEIGHT && rGridZ >= 0 && rGridZ < MAP_SIZE) {
                if (mapData[rGridX][rGridY][rGridZ] === WATER)
                    isRemoteSwimming = true;
            }
